...
{
	public class PolyhedrusPlugin : IAudioDevice
	{
		...
		public void ProcessSample(IntPtr input, IntPtr output, uint inChannelCount, uint outChannelCount, uint bufferSize)
		{
			...
			if (inChannelCount != 0)
				throw new Exception("InChannelCount for Polyhedrus must be 0");
			if (outChannelCount != 2)
				throw new Exception("OutChannelCount for Polyhedrus must be 2");
